...
export default function DeployPage() {
  ...
  const updateEnvVar = (id: string, field: "key" | "value", val: string) => {
    setEnvVars(envVars.map((env) => (env.id === id ? { ...env, [field]: val } : env)));
  };
  ...
  const parseRawEnv = (raw: string): EnvVar[] => {
    ...
    for (const line of lines) {
      ...
      if (key) result.push({ key, value, id: Date.now().toString() + Math.random() });
    }
    return result;
  };
    ...
    try {
      ...
      const res = await api.post(
        `${import.meta.env.VITE_BACKEND_URI}/project`,
        {
          ...
          repoName,
          ...(subdomainMode === "custom" && customSlug ? { userSlug: customSlug } : {}),
  ...
  useEffect(() => {
    return () => { initTimersRef.current.forEach(clearTimeout); };
  }, []);
      ...
      >
        ...
        <header className="h-14 border-b border-[#1f1f1f] flex items-center justify-between px-6 sticky top-0 z-50 bg-[rgba(10,10,10,0.85)] backdrop-blur-md">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[15px] font-semibold tracking-tight text-[#ededed]">cloudKit</span>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-5 mb-6">
            ...
            <div className="md:col-span-3 flex flex-col gap-4">
              <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-5">
                ...
                {/* Repository */}
                <div className="text-[11px] uppercase tracking-[0.15em] text-[#444] mb-3">Repository</div>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-[#1a1a1a] border border-[#222] flex items-center justify-center flex-shrink-0">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" className="text-[#777]">
                  ...
                  <div className="min-w-0">
                    <div className="text-[14px] font-semibold text-[#ededed] truncate">{repoName}</div>
                    <div className="text-[11px] text-[#555] truncate mt-0.5 font-mono">{githubRepoURL}</div>
                  </div>
                </div>

                {/* ── Subdomain ── */}
                <div className="mt-5 pt-4 border-t border-[#1a1a1a]">
                  <div className="text-[11px] uppercase tracking-[0.15em] text-[#444] mb-3">Subdomain</div>

                  <div className="flex gap-1 bg-[#0d0d0d] rounded-lg p-1 w-fit mb-3">
                    {(["auto", "custom"] as const).map((m) => (
                      <button
                        key={m}
                        onClick={() => { if (hasDeployed) return; setSubdomainMode(m); setCustomSlugError(""); }}
                        disabled={hasDeployed}
                        className="px-3.5 py-1 rounded-md text-[12px] font-medium transition-all duration-150 capitalize"
                        style={{
                          background: subdomainMode === m ? "#ededed" : "transparent",
                  ...
                  {subdomainMode === "auto" ? (
                    <div className="flex items-center bg-[#0d0d0d] border border-[#1e1e1e] rounded-lg overflow-hidden">
                      <span className="text-[13px] font-mono text-[#555] px-3 py-2 italic">auto-generated</span>
                      <span className="text-[13px] text-[#3a3a3a] py-2 pr-3">.cloud-kit.app</span>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-1.5">
                      <div
                        ...
                        style={{
                          border: customSlugError ? "1px solid #e24b4a" : "1px solid #333",
                          background: "#0d0d0d",
                        }}
                      >
                        <input
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    >
                      ...
                      <svg
                        ...
                        style={{ transform: showEnvSection ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}
                      >
                        <path d="M6 9l6 6 6-6" />
                      </svg>
                    </button>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-1 bg-[#0d0d0d] rounded-lg p-1 w-fit">
                        {(["form", "raw"] as const).map((m) => (
                          <button
                            key={m}
                            onClick={() => { if (hasDeployed) return; setEnvInputMode(m); setRawEnvError(""); }}
                            disabled={hasDeployed}
                            className="px-3 py-1 rounded-md text-[11px] font-medium transition-all duration-150 capitalize"
                            style={{
                              background: envInputMode === m ? "#222" : "transparent",
                      ...
                      {envInputMode === "raw" ? (
                        ...
                        <div className="flex flex-col gap-2">
                          <textarea
                            value={rawEnvText}
                            onChange={(e) => { setRawEnvText(e.target.value); setRawEnvError(""); }}
                            disabled={hasDeployed}
                            ...
                            style={{ cursor: hasDeployed ? "not-allowed" : "text", lineHeight: "1.6" }}
                          />
                          {rawEnvError && (
                            <span className="text-[11px] text-[#e24b4a] pl-1 flex items-center gap-1">
                              <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor">
                        ...
                        <div className="space-y-2">
                          {envVars.length === 0 && (
                            <p className="text-[11px] text-[#444] pl-1">
                              No variables added yet. Use Raw mode to paste from .env or add manually below.
                            </p>
                          )}
                          {envVars.map((env) => (
          ...
          <div className="bg-[#0d0d0d] border border-[#1e1e1e] rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-[#1a1a1a]">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-[11px] text-[#444] tracking-wide uppercase ml-1">Build Logs</span>
              </div>
              <div className="flex items-center gap-3">
            ...
            >
              {!hasDeployed ? (
                <div className="h-full flex flex-col items-center justify-center gap-2 select-none">
                  ...
                  <span className="text-[12px] text-[#333]">Press Deploy Now to start</span>
                </div>
              ) : realLogsStarted ? (

  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      clouds.forEach((c) => {
        ...
        const g = ctx.createRadialGradient(c.x, bobY, 0, c.x, bobY, Math.max(c.w, c.h));
...
export default function DashboardPage() {
  ...
  const visibleRepos   = filteredRepos.slice(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE);
      ...
      >
        ...
        <header className="h-14 border-b border-[#1f1f1f] flex items-center justify-between px-6 sticky top-0 z-50 bg-[rgba(10,10,10,0.85)] backdrop-blur-md">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-md flex items-center justify-center text-sm font-bold text-black bg-gradient-to-br from-white to-[#888]">▲</div>
            <span className="text-[15px] font-semibold tracking-tight text-[#ededed]">DeployKit</span>
          </div>
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            >
              ...
              <div className="hidden sm:flex flex-col items-start leading-tight">
                <span className="text-[13px] font-medium text-[#ededed]">{user?.fullname ?? "Loading…"}</span>
                <span className="text-[11px] text-[#666]">{user?.email ?? ""}</span>
              </div>
              <span
                className="text-[10px] text-[#555] transition-transform duration-200"
                style={{ display: "inline-block", transform: showUserMenu ? "rotate(180deg)" : "rotate(0deg)" }}
              >▾</span>
            </button>
            ...
            {showUserMenu && (
              <div className="animate-dropIn absolute top-[calc(100%+6px)] right-0 min-w-[200px] bg-[#111] border border-[#222] rounded-[10px] overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
                <div className="px-3.5 py-3 border-b border-[#1e1e1e]">
                  <div className="text-[13px] font-medium text-[#ededed]">{user?.fullname}</div>
                  <div className="text-[11px] text-[#555] mt-0.5">{user?.email}</div>
                </div>
        ...
        <main className="max-w-[1200px] mx-auto px-6 pt-12 pb-16">
          ...
          <div className="mb-12">
            ...
            <p className="text-sm text-[#555] mt-1.5">Deploy a new project or import an existing repository.</p>
          </div>
          ...
          <div className="flex items-center gap-3 bg-[#111] border border-[#222] rounded-[10px] px-4 py-3 mb-5 transition-colors duration-200 focus-within:border-[#444]">
            <svg className="text-[#444] flex-shrink-0" width="15" height="15" viewBox="0 0 15 15" fill="none">
              <path d="M7.5 1L14 7.5L7.5 14M1 7.5h13" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
            </svg>
            <input
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                {filteredRepos.length > 0 && (
                  <span className="text-[11px] text-[#444]">
                    {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, filteredRepos.length)} of {filteredRepos.length}
                  </span>
                )}
              </div>
              ...
              <div className="flex gap-2 mb-3">
                {/* Account selector */}
                <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-[#222] bg-[#111] min-w-0 flex-shrink-0" style={{ minWidth: 0, width: "48%" }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="text-[#888] flex-shrink-0">
                ...
                <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl border border-[#222] bg-[#111] flex-1 transition-colors duration-150 focus-within:border-[#333]">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-[#555] flex-shrink-0">
                ...
                {repos.length === 0
                  ? [1, 2, 3, 4, 5].map((i) => (
                      <div key={i} className="flex items-center gap-3 px-3.5 py-3">
                        ...
                        <div className="flex-1 flex flex-col gap-1.5">
                          ...
                          <div className="animate-shimmer h-2.5 rounded" style={{ width: "30%" }} />
                        </div>
                      </div>
                    ))
                  ...
                  ? (
                      <div className="px-3.5 py-8 text-center text-[13px] text-[#444]">
                        No repositories match "<span className="text-[#666]">{search}</span>"
                      </div>
                    )
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="min-w-0 flex items-center gap-1.5 flex-wrap">
                            <span className="text-[13px] font-medium text-[#ccc] truncate">{repo.name}</span>
                            {repo.private && (
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" className="text-[#555] flex-shrink-0">
                                <rect x="3" y="11" width="18" height="11" rx="2" stroke="currentColor" strokeWidth="2"/>
                                <path d="M7 11V7a5 5 0 0 1 10 0v4" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                              </svg>
                            )}
                            {repo.created_at && (
                              <span className="text-[13px] text-[#555]">· {formatDate(repo.created_at)}</span>
                            )}
                          </div>
                        </div>
                        <button
                          ...
                          onClick={(e) => { e.stopPropagation(); handleImport(repo); }}
                        >
                          Import
                        </button>
                      </div>
                    ...
                    {Array.from({ length: filteredPages }).map((_, idx) => (
                      <button
                        ...
                        className={`h-1.5 rounded-full transition-all duration-150 cursor-pointer border-0 p-0 ${
                          idx === page ? "bg-[#888] w-4" : "bg-[#333] w-1.5 hover:bg-[#555]"
                        }`}
                      />
                    ))}
                  </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="text-base font-semibold text-[#ededed] tracking-tight">Live Environment</span>
              ...
              {/* Canvas panel */}
              <div className="relative rounded-xl overflow-hidden border border-[#1e1e1e] bg-[#07080f]" style={{ height: 320 }}>
                <CloudCanvas />
                ...
                <div className="absolute bottom-0 left-0 right-0 flex items-center justify-around px-4 py-3 border-t border-[#ffffff08] bg-[rgba(7,8,15,0.7)] backdrop-blur-sm">
                  {[
                    ...
                    <div key={label} className="flex flex-col items-center gap-0.5">
                      <span className="text-[13px] font-semibold text-[#d0d0d0]">{value}</span>
